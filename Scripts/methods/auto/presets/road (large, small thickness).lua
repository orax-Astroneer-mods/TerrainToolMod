...
return {
    doDeformation = function(p)
        ...
        for i = 1, width, 100 do
            local rightMult = scale + i
            local loc2 = {
                X = loc.X + p.right.X * rightMult,
                Y = loc.Y + p.right.Y * rightMult,
                Z = loc.Z + p.right.Z * rightMult,
            }
            ...
            controller:ClientDoDeformation(
                {
                    ...
                    Instigator = nil,
                    Intensity = 5,
                    Location = loc2, ---@diagnostic disable-line: assign-type-mismatch
                    MaterialIndex = materialIndex,
                    Normal = { X = newNormal_vec3.x, Y = newNormal_vec3.y, Z = newNormal_vec3.z },
                    Operation = EDeformType.Flatten,
                    Scale = scale,
