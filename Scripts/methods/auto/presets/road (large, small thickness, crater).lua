...
return {
    doDeformation = function(p)
        ...
        ---@param location FVector_NetQuantize100
        local function doDeformation(location, operation, deformScale)
        ...
        doDeformation({
            X = loc.X + p.forward.X * (crater_scale + scale),
            Y = loc.Y + p.forward.Y * (crater_scale + scale),
            Z = loc.Z + p.forward.Z * (crater_scale + scale),
        }, EDeformType.Crater, crater_scale)

        doDeformation(loc, EDeformType.Flatten, scale)

        for i = step, width, step do
            local rightMult = scale + i
            local rX = p.right.X * rightMult
            local rY = p.right.Y * rightMult
            local rZ = p.right.Z * rightMult

            doDeformation({
                X = loc.X + p.forward.X * (crater_scale + scale) + p.right.X * rightMult,
                Y = loc.Y + p.forward.Y * (crater_scale + scale) + p.right.Y * rightMult,
                Z = loc.Z + p.forward.Z * (crater_scale + scale) + p.right.Z * rightMult,
            }, EDeformType.Crater, crater_scale)
